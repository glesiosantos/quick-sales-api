...
import { ConvertBudgetInOrderService } from '../service/convert_budget_in_order_service'
...
export class OrderController {
  ...
  async convertBudgetToOrder(request: Request, response: Response): Promise<Response> {
    const budgetInOrderService = new ConvertBudgetInOrderService()
    const order = await budgetInOrderService.closeOrder(request.params.id)
